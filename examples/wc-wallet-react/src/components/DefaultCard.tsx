...
export type MotionBoxProps = Omit<ChakraProps, keyof MotionProps> & MotionProps & {
  as?: React.ElementType;
};

export default function DefaultCard(props: DividerProps & {
  openConnectingDapp: () => any, openRequest: (request: TSessionAndTRequest) => any
}): any {
  ...
  return (
    <Flex direction="column" align="center" { ...dividerProps }>
      ...
      { walletConnectCtx.sessions.map(session => (
        <Flex key={ `s${ session.topic }` } data-testid="default-card_sesseion-card" direction="column" bg="#252b36" w="23rem" boxShadow="lg"
              p="0.5rem" my="0.5rem">
          <Flex>
          ...
          {walletConnectCtx.requests.map(requestEvent => requestEvent.topic === session.topic && (
              <Button
                ...
                bgClip="padding-box" cursor="pointer" mx="0.5rem" mt="1rem"
