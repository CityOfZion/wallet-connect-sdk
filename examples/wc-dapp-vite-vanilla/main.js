...
const init = async () => {
  ...
  // start the sdk
  wcSdk = new WcSdk(await SignClient.init({
...
const connect = async () => {
  ...
  await wcSdk.connect('neo3:testnet', ['invokeFunction', 'testInvoke', 'signMessage', 'verifyMessage', 'traverseIterator', 'getWalletInfo', 'getNetworkVersion'])
  renderAfterConnect()
}
...
const signAndVerify = async () => {
  if (!wcSdk) return
  const resp = await wcSdk.signMessage({ message: 'Carai, muleq, o baguiu eh issumermo taix ligads na missaum?', version: Version.DEFAULT })
...
const signWithoutSaltAndVerify = async () => {
  if (!wcSdk) return
  const resp = await wcSdk.signMessage({ message: 'Carai, muleq, o baguiu eh issumermo taix ligads na missaum?', version: Version.WITHOUT_SALT })
