import React$1, { useState, useCallback, useEffect, useContext } from 'react';
  ...
  ReactCurrentDispatcher: {
    current: {
      readContext: function readContext(n) {
        return q$2.__n[n.__c].props.value;
      }
    }
  }
},
...
var detectBrowser = getCjsExportFromNamespace(es);

var React = getCjsExportFromNamespace(compat_module$1);
...
var WalletConnectContext = React$1.createContext({});
var WalletConnectContextProvider = function WalletConnectContextProvider(_ref) {
  ...
  var initWcClient = useCallback(function () {
    try {
      return Promise.resolve(WcSdk.initClient(options.logger, options.relayServer)).then(function (_WcSdk$initClient) {
  ...
  var resetApp = useCallback(function () {
    try {
  ...
  var checkPersistedState = useCallback(function () {
    try {
      ...
      return Promise.resolve(WcSdk.getSession(wcClient)).then(function (s) {
        if (s) {
  ...
  var connect = function connect(topic) {
    try {
      ...
      var _temp4 = _catch(function () {
        return Promise.resolve(WcSdk.connect(wcClient, _extends({
          topic: topic
  ...
  var disconnect = function disconnect() {
    try {
      ...
      return Promise.resolve(WcSdk.disconnect(wcClient, session)).then(function () {
  ...
  var sendRequest = function sendRequest(request) {
    return Promise.resolve(handleRequest(function (c, s) {
      try {
        return Promise.resolve(WcSdk.sendRequest(c, s, options.chainId, request));
      } catch (e) {
        return Promise.reject(e);
      }
    }));
  ...
  var invokeFunction = function invokeFunction(scripthash, method, params) {
    return Promise.resolve(handleRequest(function (c, s) {
      try {
        return Promise.resolve(WcSdk.invokeFunction(c, s, options.chainId, scripthash, method, params));
      } catch (e) {
        return Promise.reject(e);
      }
    }));
  ...
  return React$1.createElement(WalletConnectContext.Provider, {
    value: contextValue
  }, children);
};
